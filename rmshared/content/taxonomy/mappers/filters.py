...
class Filters(IFilters[Filter]):
    ...
    def __init__(self, labels: ILabels, ranges: IRanges):
        self.labels = labels
        self.ranges = ranges
        self.filter_to_factory_func_map: Mapping[Type['Filters.AnyFilter'], Callable[['Filters.AnyFilter'], core.filters.Filter]] = {
    ...
    def _map_filter(self, filter_: Filter) -> core.filters.Filter:
    ...
    def _map_any_range_filter(self, filter_: filters.AnyRange) -> core.filters.AnyRange:
        return core.filters.AnyRange(ranges=tuple(map(self.ranges.map_range, filter_.ranges)))

    def _map_no_ranges_filter(self, filter_: filters.NoRanges) -> core.filters.NoRanges:
        return core.filters.NoRanges(ranges=tuple(map(self.ranges.map_range, filter_.ranges)))
