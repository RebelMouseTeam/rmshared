...
class Ranges(IRanges):
    ...
    def map_range(self, range_):
        ...
        if None not in {range_.min_value, range_.max_value}:
            yield core0.ranges.Between(field, min_value=range_.min_value, max_value=range_.max_value)
        elif range_.max_value is not None:
            yield core0.ranges.LessThan(field, value=range_.max_value)
        elif range_.min_value is not None:
            yield core0.ranges.MoreThan(field, value=range_.min_value)
        else:
            return []
